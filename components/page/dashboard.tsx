...
import { FcAndroidOs, FcFlashOn, FcIphone, FcPhone, FcPhoneAndroid, FcWiFiLogo } from "react-icons/fc";
...
export default function DashboardPage(props: DashboardProps) {
  ...
  <div className="w-full max-w-screen-xl space-y-6">
    {/* Navbar */}
    <nav className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-700 pb-4">
      ...
      <Tabs defaultValue="realtime">
        <TabsList className="bg-zinc-100 dark:bg-zinc-800 rounded-xl shadow-inner">
    ...
    <div className="flex items-center gap-2 mb-4">
      ...
      <div className="p-6">
      {/* List */}
        <div className="max-h-64 overflow-y-auto mt-2 divide-y divide-zinc-200 dark:divide-zinc-700 pr-2">
          {dashboardData.clicks.map((click, index) => (
            <div
              key={click.id}
              className={`flex items-center justify-between py-2 text-sm text-zinc-800 dark:text-zinc-200 
                        transition duration-500 ease-in-out hover:shadow px-2 rounded-md 
                        blink animate-pulse`}
      ...
      <Card className="rounded-2xl shadow-lg transition hover:shadow-2xl hidden lg:block">
        <div className="p-6">
          <h2 className="font-semibold text-xl text-zinc-800 dark:text-white mb-2">
            Top Users
          </h2>
          {dashboardData.topUsers.length === 0 ? (
            ...
            <ul className="space-y-1 mt-2 text-zinc-700 dark:text-zinc-200">
              {dashboardData.topUsers.slice(0, 3).map((user, i) => (
                <li key={i}>
              ...
              <td className="p-3">
                {lead.useragent.includes("WAP") ? (
                  ...
                  <Image
                    ...
                    alt="Source Icon"
                    className="inline-block mr-1"
                  /> // bisa diganti dengan elemen lain
                )}
              </td>
              <td className="p-3">{lead.ip}</td>
              <td className="p-3 text-green-600 dark:text-green-400 font-medium">${lead.earning.toFixed(2)}</td>
              <td><ClientDate date={lead.created_at}/></td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
      <TopCountryChart countryData={dashboardData.countryData} />
      <Card className="rounded-2xl shadow-lg transition hover:shadow-2xl">
        <CardContent className="p-6">
          ...
          {dashboardData.topLeads.length === 0 ? (
            ...
            <ul className="space-y-1 text-zinc-700 dark:text-zinc-200">
              {dashboardData.topLeads.map((lead, i) => (
                <li key={i}>{i + 1}. {lead.name} - ${lead.total.toFixed(2)}</li>
              ))}
            </ul>
          )}
        </CardContent>
